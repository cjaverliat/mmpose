...
class BaseMMPoseInferencer(BaseInferencer):
    ...
    def _get_webcam_inputs(self, inputs: str) -> Generator:
        ...
        assert inputs.startswith("webcam"), (
            f'Expected input to start with "webcam", but got "{inputs}"'
        )
        ...
        if len(inputs_) == 1:
            ...
            raise ValueError(
                f'Expected webcam input to have format "webcam:id", but got "{inputs}"'
            )
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Callable:
        ...
        load_img_idx = self._get_transform_idx(pipeline_cfg, ("LoadImage", LoadImage))
        if load_img_idx == -1:
            raise ValueError("LoadImage is not found in the test pipeline")
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        if isinstance(inputs, torch.Tensor):
            assert batch_size == inputs.shape[0], (
                f"Batch size {batch_size} does not match number of images {inputs.shape[0]}"
            )

            if hasattr(self, "preprocess_batch"):
                data_infos = self.preprocess_batch(
                    inputs, batch_size=batch_size, bboxes=bboxes, bbox_thr=bbox_thr, nms_thr=nms_thr, **kwargs
                )
            else:
                data_infos = []
                ...
                for i in range(batch_size):
                    ...
                    data_infos.extend(self.preprocess_single(
                        inputs[i],
                        index=i,
                        bboxes=bbox,
                        bbox_thr=bbox_thr,
            ...
            yield self.collate_fn(data_infos), inputs
        else:
